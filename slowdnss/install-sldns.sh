...
service ssh restart
sudo echo "screen -dmS slowdns iptables -I INPUT -p udp --dport 5300 -j ACCEPT && iptables -t nat -I PREROUTING -p udp --dport 53 -j REDIRECT --to-ports 5300 && ulimit -u unlimited && ulimit -n 65536 && /root/dnstt/dnstt-server/dnstt-server -udp :5300 -privkey-file /root/dnstt/dnstt-server/server.key $nameserver 127.0.0.1:443" > /bin/slow
...
#vsystemctl enable server-sldns
#systemctl start client-sldns
#vsystemctl start server-sldns
